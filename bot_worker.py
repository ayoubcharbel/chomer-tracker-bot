"""
Telegram Activity Tracker Bot - Background Worker Version
Optimized for Render Background Worker deployment
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"User {user.first_name} ({user.id}) earned {POINTS_PER_MESSAGE} point(s) for a message")
...
async def handle_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"User {user.first_name} ({user.id}) earned {POINTS_PER_STICKER} point(s) for a sticker")

def main() -> None:
    """Start the bot"""
    try:
        logger.info("🚀 Starting Telegram Activity Tracker Bot...")
